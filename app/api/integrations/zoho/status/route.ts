...
export async function GET(request: NextRequest) {
  try {
    ...
    const integration = await getGlobalIntegration(userId, orgId || null, 'zoho')
    ...
    if (!integration) {
      ...
      try {
        ...
        console.error('[Zoho Status] Error checking Composio connections:', composioError)
      }
    ...
    if (connectionId) {
      try {
        ...
        console.error('[Zoho Status] Error fetching connection:', error)
        return NextResponse.json({
          connected: false,
          syncedContactsCount: integration.syncedContactsCount || 0,
    ...
    console.error('[Zoho Status] Unexpected error:', error)
    return NextResponse.json(
      { 
