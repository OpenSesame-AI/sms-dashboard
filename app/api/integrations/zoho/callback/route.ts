...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      return NextResponse.redirect(
        new URL(`/?error=zoho_oauth_error&error_description=${encodeURIComponent(error)}`, request.url)
      )
    }
    ...
    try {
      ...
      try {
        ...
        // If that doesn't work, list user's connections and find the Zoho one
        ...
        const zohoConnection = connections.find((conn: any) => {
          // Check toolkit field
          if (conn.toolkit) {
        ...
        if (!zohoConnection) {
          throw new Error('Zoho connection not found. Please try connecting again.')
        }
    ...
    return NextResponse.redirect(
      new URL('/?zoho_connected=true', request.url)
    )
  } catch (error) {
