...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      return NextResponse.redirect(
        new URL(`/?error=zendesk_oauth_error&error_description=${encodeURIComponent(error)}`, request.url)
      )
    }
    ...
    try {
      ...
      try {
        ...
        // If that doesn't work, list user's connections and find the Zendesk one
        ...
        const zendeskConnection = connections.find((conn: any) => {
          // Check toolkit field
          if (conn.toolkit) {
          ...
          return conn.appUniqueId === 'ZENDESK' || 
        ...
        if (!zendeskConnection) {
          throw new Error('Zendesk connection not found. Please try connecting again.')
        }
        
        connectionId = zendeskConnection.id
      }
    } catch (err) {
    ...
    return NextResponse.redirect(
      new URL('/?zendesk_connected=true', request.url)
    )
  } catch (error) {
