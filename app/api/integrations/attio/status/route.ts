...
export async function GET(request: NextRequest) {
  try {
    ...
    const integration = await getGlobalIntegration(userId, orgId || null, 'attio')
    ...
    if (!integration) {
      ...
      try {
        ...
        console.error('[Attio Status] Error checking Composio connections:', composioError)
      }
    ...
    if (connectionId) {
      try {
        ...
        console.error('[Attio Status] Error fetching connection:', error)
        return NextResponse.json({
          connected: false,
          syncedContactsCount: integration.syncedContactsCount || 0,
    ...
    console.error('[Attio Status] Unexpected error:', error)
    return NextResponse.json(
      { 
