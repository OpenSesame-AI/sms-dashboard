...
import { createOrUpdateGlobalIntegrationWithConnectionId, getGlobalIntegration, updateIntegration } from '@/lib/db/queries'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // State format: base64(randomBytes)
    const randomState = randomBytes(16).toString('base64')
    const state = Buffer.from(randomState).toString('base64')
    ...
    // If immediate (API Key), create global integration directly
    if (immediate && connectionRequestId) {
      // For API Key auth, the connection is already established
      // Create the global integration record in the database
      try {
