...
import { fetchContacts, extractPhoneNumbers } from '@/lib/zoho-bigin'
import { composio, isConnectionStatusActive } from '@/lib/composio'
import { db } from '@/lib/db/index'
import { phoneUserMappings, zohoBiginContacts } from '@/lib/db/schema'
...
export async function POST(request: NextRequest) {
  try {
    ...
    let integration = await getGlobalIntegration(userId, orgId || null, 'zoho_bigin')
    let connectionId: string | null = null
    
    if (!integration) {
      // No integration in database - check if Zoho Bigin connection exists in Composio
      console.log(`[Zoho Bigin Sync] No integration found in database, checking Composio for connections...`)
      ...
      try {
        ...
        // Find Zoho Bigin connections
        const zohoBiginConnections = connections.filter((conn: any) => {
          ...
          if (conn.appUniqueId === 'ZOHO_BIGIN' || 
          ...
          if (conn.authConfig) {
            ...
            if (authConfig.appUniqueId === 'ZOHO_BIGIN' ||
                authConfig.appName === 'ZOHO_BIGIN' ||
                (authConfig.appUniqueId && authConfig.appUniqueId.toLowerCase().includes('zoho_bigin')) ||
                (authConfig.appUniqueId && authConfig.appUniqueId.toLowerCase().includes('zoho-bigin'))) {
              return true
            }
          }
        ...
        if (zohoBiginConnections.length === 0) {
          return NextResponse.json(
            { error: 'Zoho Bigin integration not connected. Please connect Zoho Bigin first.' },
            { status: 400 }
          )
        }
        
        // Use the first active Zoho Bigin connection
        const firstConnection = zohoBiginConnections[0]
        ...
        if (!isActive) {
          return NextResponse.json(
            { error: `Zoho Bigin connection found but status is: ${connectionStatus}. Please reconnect.` },
            { status: 400 }
          )
        }
        ...
        if (!connectionId) {
          return NextResponse.json(
            { error: 'Zoho Bigin connection found but missing connection ID' },
            { status: 400 }
          )
        }
        console.log(`[Zoho Bigin Sync] Auto-linking Zoho Bigin connection ${connectionId} to global integration`)
        ...
        console.log(`[Zoho Bigin Sync] Created global integration record: ${integration.id}`)
      } catch (error) {
        console.error('[Zoho Bigin Sync] Error checking Composio connections:', error)
        return NextResponse.json(
          { error: 'Failed to check Zoho Bigin connection. Please try again.' },
          { status: 500 }
        )
      }
    } else {
    ...
    if (!connectionId) {
      return NextResponse.json(
        { error: 'Zoho Bigin connection ID not found' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      console.error('Error fetching contacts from Zoho Bigin:', error)
      ...
      if (error instanceof Error && (
        ...
        return NextResponse.json(
          { error: 'Connection expired. Please reconnect to Zoho Bigin.' },
          { status: 401 }
        )
      }
      ...
      for (const contact of contacts) {
        ...
        for (const phoneNumber of phoneNumbers) {
          ...
          if (!existing) {
            ...
            // Store Zoho Bigin contact data
            const existingBiginContact = await db
              .select()
              .from(zohoBiginContacts)
              .where(
                and(
                  eq(zohoBiginContacts.phoneNumber, normalizedPhone),
                  eq(zohoBiginContacts.cellId, cell.id)
                )
              )
              .limit(1)
            
            if (existingBiginContact[0]) {
              // Update existing Zoho Bigin contact
              await db
                .update(zohoBiginContacts)
                .set({
                  zohoBiginId: contact.id,
                  firstName: contact.First_Name || null,
                  lastName: contact.Last_Name || null,
                  email: contact.Email || null,
                  companyName: contact.Account_Name?.name || null,
                  updatedAt: new Date(),
                })
                .where(eq(zohoBiginContacts.id, existingBiginContact[0].id))
            } else {
              // Insert new Zoho Bigin contact
              await db.insert(zohoBiginContacts).values({
                phoneNumber: normalizedPhone,
                cellId: cell.id,
                zohoBiginId: contact.id,
                firstName: contact.First_Name || null,
            ...
            // Update Zoho Bigin contact data
            const existingBiginContact = await db
              .select()
              .from(zohoBiginContacts)
              .where(
    ...
    return NextResponse.json({
      ...
      message: `Synced ${totalSyncedCount} new contacts${totalUpdatedCount > 0 ? ` and updated ${totalUpdatedCount} existing contacts` : ''} from Zoho Bigin to ${allCells.length} cell${allCells.length > 1 ? 's' : ''}`,
    })
  } catch (error) {
