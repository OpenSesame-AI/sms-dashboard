...
import { getOrCreateZohoBiginAuthConfig, initiateZohoBiginConnection } from '@/lib/composio'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const authConfigId = await getOrCreateZohoBiginAuthConfig()
    ...
    const { connectionRequestId, redirectUrl } = await initiateZohoBiginConnection(
      userId,
      authConfigId
    )
    ...
    console.error('Error initiating Zoho Bigin connection:', error)
    return NextResponse.json(
      { 
