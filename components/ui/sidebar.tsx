...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const openState = typeof value === "function" ? value(openProp ?? open) : value
      if (setOpenProp) {
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (collapsible === "offcanvas") {
    if (state === "expanded") {
      ...
      sidebarStyle.marginLeft = side === "left" ? "0" : undefined
      sidebarStyle.marginRight = side === "right" ? "0" : undefined
    } else {
      sidebarStyle.width = SIDEBAR_WIDTH
      sidebarStyle.marginLeft = side === "left" ? `-${SIDEBAR_WIDTH}` : undefined
      sidebarStyle.marginRight = side === "right" ? `-${SIDEBAR_WIDTH}` : undefined
    }
  } else if (collapsible === "icon") {
    sidebarStyle.width = state === "expanded" ? SIDEBAR_WIDTH : SIDEBAR_WIDTH_ICON
  }
  ...
  return (
    <div
      ...
      data-sidebar={variant}
      data-side={side}
      data-collapsible={collapsible === "offcanvas" ? "offcanvas" : state === "collapsed" ? "icon" : state}
      data-state={state}
      data-mobile={isMobile}
      className={cn(
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:-translate-x-1/2 after:bg-sidebar-border after:opacity-0 after:transition-opacity after:duration-200 hover:after:opacity-100 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 peer-data-[collapsible=offcanvas]:peer-data-[state=collapsed]:group-data-[side=left]:-right-2 peer-data-[collapsible=offcanvas]:peer-data-[state=collapsed]:group-data-[side=right]:-left-2 md:flex",
          "peer-data-[collapsible=offcanvas]:peer-data-[state=collapsed]:cursor-pointer",
          className
        )}
        {...props}
      />
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "md:peer-data-[collapsible=offcanvas]:md:group-data-[side=left]:md:ml-0",
        "md:peer-data-[collapsible=offcanvas]:md:group-data-[side=right]:md:mr-0",
        "md:duration-200 md:transition-[margin] md:ease-linear",
        className
      )}
      {...props}
    />
...
>(({ asChild = false, showOnHover = false, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1 top-1.5 flex aspect-square w-6 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-data-[size=sm]/menu-button:top-1 peer-data-[size=lg]/menu-button:top-2.5 [&>svg]:size-4 [&>svg]:shrink-0",
        "peer-data-[state=open]/menu-button:bg-sidebar-accent peer-data-[state=open]/menu-button:text-sidebar-accent-foreground",
        showOnHover &&
          "group-focus-within/menu-item:opacity-100 group-data-[active=true]/menu-item:opacity-100 peer-data-[active=true]/menu-button:opacity-100 opacity-0 transition-opacity group-hover/menu-item:opacity-100",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
>(({ className, showIcon = false, ...props }, ref) => {
  return (
    <div
      ...
      className={cn("rounded-md h-8 flex gap-2 px-2 items-center bg-sidebar-accent/10", className)}
      {...props}
    >
